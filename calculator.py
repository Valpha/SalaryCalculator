...
def calculate(salary: int, span: int, rate: float, mode="yearly", deposit=0):
    if mode == "yearly":
        X, base_deposit = Base(salary=salary, span=span,
                               mode=mode, deposit=deposit)
...
def drawPlot(X, base, profit, salary, rate, deposit):
    ...
    title = "你    的    银    行\n"+"年薪："+str(round(salary/10000, 1))+"万 目前积蓄："+str(round(deposit/10000, 1))+"万 年化率："+str(
    ...
    plt.yticks(np.arange(0, profit.max()+base[1], step=step), (np.arange(
        0, (profit.max()+base[1])/10000, step=step/10000).tolist()))
    ...
    for x, y, z in zip(X, profit, base):
        if x == 0:
            continue
        plt.text(x, y+0.3, str(round(y/10000, 1))+' +'+str(round((y-z)/10000, 1))+',' +
                 str(round((y-z)/z*100, 1))+"%", ha='center', va='center', fontsize=12, rotation=0, zorder=10)
    plt.grid(True)
    ...
    buffer.seek(0)
    figdate_png = base64.b64encode(buffer.getvalue())
    # image_cache_name = makeImageCache(salary, len(X), rate, deposit)
    # plt.savefig(image_cache_name)
    
    return figdate_png


# def makeImageCache(salary, span, rate, deposit: int = 0):
    # base_string = str(salary) + str(span) + str(rate) + str(deposit)
    # cache_bytes = base64.urlsafe_b64encode(bytes(base_string, encoding="utf8"))
    
    # cache_string = str(cache_bytes, encoding = "utf-8")
    # return './app/static/'+cache_string
...
# buffer = drawPlot(X, base, profit, salary=Salary_year, rate=rate, deposit=deposit)
# print(buffer)
